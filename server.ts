...
async function startServer() {
  ...
  // Helper function to resolve output directory based on environment (CI vs Local Windows)
  function resolveOutputDirectory(): string {
    if (isCi) {
      // Use local project output folder for GitHub Actions runners
      return path.join(process.cwd(), 'output');
    }
    // Use local Windows Google Drive path when running locally
    return 'G:\\My Drive\\Daily_Options_Trades';
  }
  ...
  if (isCi) {
    ...
    try {
      ...
      console.error('[SERVER] Pipeline execution or report generation failed in CI:', err);
      process.exit(1);
    }
  }
}
